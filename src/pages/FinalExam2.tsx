...
export default function FinalExam2() {
  ...
  const [timerMode, setTimerMode] = useState<number | null>(null) // minutes or null for no timer
  const [timeUp, setTimeUp] = useState(false)

  const questions = useFinalExam(2)
  ...
  const handleNext = () => {
    if (current < questions.length - 1) {
      ...
      setFinished(true)
      useProgressStore.getState().submitExamScore('examen-programme-2', finalPct)
      ...
      <div style={{ paddingTop: 56 }}>
        <div style={{ maxWidth: 640, margin: '0 auto', padding: '60px 24px 80px' }}>
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} style={{ textAlign: 'center' }}>
            ...
            }}>{t('// examen programme 2', '// program 2 exam')}</div>

            <h1 style={{
              fontFamily: 'var(--font-heading)', fontSize: 36, fontWeight: 700,
              letterSpacing: '-1px', marginBottom: 16,
            }}>{t('Examen - Programme 2', 'Exam - Program 2')}</h1>
            ...
            <p style={{
              ...
              {t(
                '50 questions sur TCP/UDP, routage dynamique, ACL, NAT/PAT, securite Wi-Fi, VPN & tunneling, SNMP et depannage.',
                '50 questions on TCP/UDP, dynamic routing, ACL, NAT/PAT, Wi-Fi security, VPN & tunneling, SNMP and troubleshooting.'
              )}
            </p>
            ...
            <div style={{
              ...
              {[
                ...
                { label: t('Chapitres', 'Chapters'), value: '8', icon: <GraduationCap size={16} /> },
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {[
                  ...
                  <button key={opt.label} onClick={() => setTimerMode(opt.value)}
                    style={{
                      padding: '6px 16px',
                      background: timerMode === opt.value ? '#6366f1' : 'var(--bg-primary)',
                      color: timerMode === opt.value ? 'white' : 'var(--text-secondary)',
                      border: `1px solid ${timerMode === opt.value ? '#6366f1' : 'var(--border)'}`,
  ...
  if (finished || timeUp) {
    ...
    const chapterNames = ['TCP/UDP', t('Routage dyn.', 'Dyn. Routing'), 'ACL', 'NAT/PAT', t('Securite Wi-Fi', 'Wi-Fi Security'), 'VPN & Tunnel.', 'SNMP', t('Depannage', 'Troubleshoot.')]
    const ranges = [[0, 6], [6, 13], [13, 20], [20, 26], [26, 32], [32, 38], [38, 44], [44, 50]]
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 700, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{ fontFamily: 'var(--font-heading)', fontSize: 15, fontWeight: 600 }}>{t('Examen Programme 2', 'Exam Program 2')}</span>
          </div>
          ...
          <motion.div key={current} initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {order.map((originalIdx, displayIdx) => {
                ...
                } else if (isThisSelected) { bg = 'rgba(99, 102, 241, 0.1)'; border = '#6366f1' }
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 24 }}>
              {!showResult ? (
                <button onClick={handleValidate} disabled={selected === null} style={{
                  padding: '10px 24px',
                  background: selected !== null ? '#6366f1' : 'var(--bg-tertiary)',
