...
export default function FinalExam3() {
  ...
  const questions = useFinalExam(3)
  ...
  const handleNext = () => {
    if (current < questions.length - 1) {
      ...
      setFinished(true)
      useProgressStore.getState().submitExamScore('examen-programme-3', finalPct)
      ...
      <div style={{ paddingTop: 56 }}>
        <div style={{ maxWidth: 640, margin: '0 auto', padding: '60px 24px 80px' }}>
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} style={{ textAlign: 'center' }}>
            ...
            }}>{t('// examen programme 3', '// program 3 exam')}</div>

            <h1 style={{
              fontFamily: 'var(--font-heading)', fontSize: 36, fontWeight: 700,
              letterSpacing: '-1px', marginBottom: 16,
            }}>{t('Examen - Programme 3', 'Exam - Program 3')}</h1>
            ...
            <p style={{
              ...
              {t(
                '50 questions sur Python, Ansible, Cloud, Kubernetes, SDN, haute disponibilite, securite avancee et monitoring.',
                '50 questions on Python, Ansible, Cloud, Kubernetes, SDN, high availability, advanced security and monitoring.'
              )}
            </p>
  ...
  if (finished || timeUp) {
    ...
    const chapterNames = ['Python', 'Ansible', 'Cloud', 'Kubernetes', 'SDN', t('Haute Dispo.', 'High Avail.'), t('Securite Av.', 'Adv. Security'), 'Monitoring']
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 700, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{ fontFamily: 'var(--font-heading)', fontSize: 15, fontWeight: 600 }}>{t('Examen Programme 3', 'Exam Program 3')}</span>
          </div>
          ...
          <motion.div key={current} initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {order.map((originalIdx, displayIdx) => {
                ...
                } else if (isThisSelected) { bg = 'rgba(245, 158, 11, 0.1)'; border = accentColor }
